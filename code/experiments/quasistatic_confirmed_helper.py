...
class QuasiStaticParams:
    ...
    desired_speed_cap: float = 0.10
    support_enabled: bool = True
    support_weight_start: float = 0.95
    support_weight_end: float = 0.55
    support_fade_start: float = 0.4
    support_fade_end: float = 2.2
    target_height_frac: float = 0.98
    target_roll: float = 0.0
    target_pitch: float = 0.0
    target_yaw: float = 0.0

    support_xy_k: float = 220.0
    support_xy_d: float = 36.0
    support_z_k: float = 520.0
    support_z_d: float = 60.0
    support_roll_k: float = 26.0
    support_roll_d: float = 3.2
    support_pitch_k: float = 22.0
    support_pitch_d: float = 2.8
    support_yaw_k: float = 5.0
    support_yaw_d: float = 1.0
    support_max_force_xy: float = 70.0
    support_max_force_z: float = 220.0
    support_max_torque: float = 20.0

    force_frame: str = "body"
    realization: str = "external"
    mpc_force_gain_start: float = 0.00
    mpc_force_gain_end: float = 0.22
    mpc_force_ramp_start: float = 0.8
    mpc_force_ramp_end: float = 2.4
    max_xy_over_fz: float = 0.35

    clearance: float = 0.085
    step_len_front: float = 0.065
    rear_step_scale: float = 0.90
    touchdown_depth_front: float = 0.015
    touchdown_depth_rear: float = 0.020
    touchdown_extra: float = 0.010
    dq_limit: float = 0.18
    damping_ls: float = 1e-4

    stance_press_front: float = 0.008
    stance_press_rear: float = 0.010
    stance_press_gain: float = 0.00045
    stance_fx_bias_gain: float = 0.0008
    stance_fy_bias_gain: float = 0.0006

    shift_x_mag: float = 0.020
    shift_y_mag: float = 0.020
    touchdown_confirm_hold: float = 0.06

    recovery_height_enter_frac: float = 0.78
    recovery_height_exit_frac: float = 0.90
    recovery_pitch_enter: float = 0.28
    recovery_roll_enter: float = 0.22
    recovery_pitch_exit: float = 0.12
    recovery_roll_exit: float = 0.10
    recovery_min_time: float = 0.18
...
class ConfirmedQuasiStaticCrawl:
    ...
    def _compute_shift_target(self, swing_leg: int):
        ...
        ctr[0] += -front_sign(swing_leg) * self.params.shift_x_mag
    ...
    def step_state_machine(self, t: float, actual_contact: np.ndarray):
        ...
        if self.phase != "RECOVERY" and self._is_unhealthy(pos, rpy):
            self.phase = "RECOVERY"
            self.phase_start = t
        ...
        if self.phase == "STARTUP":
            ...
            # refresh anchors from all feet to stop drift
            ...
            self.swing_leg = None
            self.body_target_xy = pos[:2].copy()
            if (t - self.phase_start) >= self.params.recovery_min_time and self._is_recovered(pos, rpy):
                self.phase = "SHIFT"
                self.phase_start = t
                self.swing_leg = int(self.params.crawl_order[self.leg_ptr])
                self.body_target_xy = self._compute_shift_target(self.swing_leg)
    ...
    def build_ctrl_targets(self, t: float, u_cmd: np.ndarray, actual_contact: np.ndarray) -> np.ndarray:
        ...
        for leg in self.bindings.leg_bindings:
            ...
            if sched[i]:
                ...
                press = (self.params.stance_press_rear if is_rear(i) else self.params.stance_press_front) + self.params.stance_press_gain * fz
                tgt[2] -= press
                # small MPC-informed horizontal bias on stance target
    ...
    def apply_support(self, t: float) -> dict:
        ...
        fz = w * (self.params.support_z_k * (target_pos[2] - pos[2]) - self.params.support_z_d * lin_vel[2])
