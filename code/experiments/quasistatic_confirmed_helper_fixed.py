...
class QuasiStaticParams:
    ...
    support_weight_end: float = 0.70
    support_fade_start: float = 0.4
    support_fade_end: float = 3.0
    target_height_frac: float = 0.82
    ...
    support_z_k: float = 340.0
    support_z_d: float = 40.0
    support_roll_k: float = 18.0
    support_roll_d: float = 2.2
    support_pitch_k: float = 16.0
    support_pitch_d: float = 2.0
    support_yaw_k: float = 5.0
    support_yaw_d: float = 1.0
    support_max_force_xy: float = 55.0
    support_max_force_z: float = 260.0
    support_max_torque: float = 20.0

    force_frame: str = "body"
    realization: str = "external"
    mpc_force_gain_start: float = 0.00
    mpc_force_gain_end: float = 0.18
    ...
    recovery_height_enter_frac: float = 0.60
    recovery_height_exit_frac: float = 0.72
    recovery_pitch_enter: float = 0.45
    recovery_roll_enter: float = 0.35
    recovery_pitch_exit: float = 0.22
    recovery_roll_exit: float = 0.18
    recovery_min_time: float = 0.12

    recovery_enable_after: float = 1.20
    recovery_force_exit_after: float = 0.55
    recovery_required_contacts: int = 3
...
class ConfirmedQuasiStaticCrawl:
    ...
    def _compute_shift_target(self, swing_leg: int):
        ...
        if is_rear(swing_leg):
            ctr[0] += 0.35 * self.params.shift_x_mag
        else:
            ctr[0] -= 0.20 * self.params.shift_x_mag
    ...
    def build_ctrl_targets(self, t: float, u_cmd: np.ndarray, actual_contact: np.ndarray) -> np.ndarray:
        ...
        for leg in self.bindings.leg_bindings:
            ...
            if sched[i]:
                ...
                rear_contact_ratio = float(np.mean(actual_contact[2:4]))
                front_bias = 0.002 if (not is_rear(i) and rear_contact_ratio < 0.5) else 0.0
