...
from phases.mujoco_phase14_support_helpers import apply_trunk_support_wrench_v2, build_phase14_summary, write_phase14_summary
...
def run_mujoco_phase14_forward_supported_visual(
    cfg,
    model_path: str,
    viewer: bool = True,
    ...
    support_x_k: float = 35.0,
    support_y_k: float = 14.0,
    support_forward_bias: float = 3.0,
    ...
    if support_target_height is None:
        support_target_height = min(max(0.29, 0.78 * nominal_trunk_height), nominal_trunk_height)
    support_x0 = float(d.body(bindings.base_body_name).xpos[0])
    support_y0 = float(d.body(bindings.base_body_name).xpos[1])
    ...
    def one_step() -> bool:
        ...
        support_target_x = support_x0 + max(0.0, float(d.time) - settle_time) * float(cfg_run.desired_speed)
        support_target_y = support_y0
        support = apply_trunk_support_wrench_v2(
            m, d, trunk_bid,
            ...
            target_x=support_target_x,
            target_y=support_target_y,
            x_k=support_x_k,
            y_k=support_y_k,
            ...
            pitch_target=support_target_pitch,
            pitch_k=support_pitch_k,
            pitch_d=support_pitch_d,
            yaw_k=support_yaw_k,
            yaw_d=support_yaw_d,
            forward_bias=float(gait_alpha) * support_forward_bias,
    ...
    if output_dir is not None:
        ...
        summary = build_phase14_summary(log, settle_time=settle_time)
        summary.update({
            ...
            'support_weight_frac': float(support_weight_frac), 'support_x_k': float(support_x_k), 'support_y_k': float(support_y_k), 'support_forward_bias': float(support_forward_bias), 'support_target_pitch': float(support_target_pitch),
        ...
        saved.append(write_phase14_summary(output_dir, summary))
...
def main() -> None:
    parser = argparse.ArgumentParser(description='Forward-tracking support-assisted MuJoCo visual demo using the same high-level Python MPC loop.')
    ...
    parser.add_argument('--support-x-k', type=float, default=35.0)
    parser.add_argument('--support-y-k', type=float, default=14.0)
    parser.add_argument('--support-forward-bias', type=float, default=3.0)
    parser.add_argument('--support-target-pitch', type=float, default=0.0)
    ...
    try:
        _, saved, summary = run_mujoco_phase14_forward_supported_visual(
            cfg=cfg,
            ...
            support_x_k=args.support_x_k,
            support_y_k=args.support_y_k,
            support_forward_bias=args.support_forward_bias,
            support_target_pitch=args.support_target_pitch,
        ...
        print(f'MuJoCo phase-14 forward-supported run finished for scenario: {cfg.scenario_name}')
        ...
        print('MuJoCo phase-14 forward-supported run failed. Full traceback below:')
