...
def _collect_subtree_bodies(m: mujoco.MjModel, root_body_id: int) -> set[int]:
    """Collect body ids in the subtree rooted at ``root_body_id``.

    The Python bindings expose ``body_parentid`` on ``MjModel`` but not the
    convenience arrays ``body_childid`` / ``body_siblingid`` that some older
    helper code assumes. Build the subtree explicitly from parent links so the
    function works across current MuJoCo Python releases.
    """
...
def _actuator_id_for_joint(m: mujoco.MjModel, joint_id: int) -> int | None:
    for aid in range(m.nu):
        ...
        # 0 corresponds to joint transmission in MuJoCo's internal enum.
        if trn_obj_kind == 0 and trn_obj_id == joint_id:
            return aid
    return None
...
def foot_point_world(d: mujoco.MjData, calf_body_name: str, local_offset: np.ndarray) -> np.ndarray:
    body = d.body(calf_body_name)
    R = body.xmat.reshape(3, 3)
    return body.xpos.copy() + R @ local_offset


def foot_rel_world(d: mujoco.MjData, base_body_name: str, leg_bindings: Iterable[LegBinding], local_offset: np.ndarray) -> np.ndarray:
    p_base = d.body(base_body_name).xpos.copy()
    feet = [foot_point_world(d, binding.calf_body_name, local_offset) - p_base for binding in leg_bindings]
...
def mujoco_to_x(d: mujoco.MjData, cfg: MPCConfig, base_body_name: str) -> np.ndarray:
    ...
    x[IDX_P] = base.xpos.copy()
    x[IDX_TH] = mat_to_rpy(base.xmat.reshape(3, 3))
...
def actual_contact_state(m: mujoco.MjModel, d: mujoco.MjData, bindings: ModelBindings) -> np.ndarray:
    ...
    for ci in range(d.ncon):
        ...
        for leg_idx, leg in enumerate(bindings.leg_bindings):
            if (g1 in leg.geom_ids and g2 in floor) or (g2 in leg.geom_ids and g1 in floor):
...
def estimate_swing_lift_dir(m: mujoco.MjModel, d: mujoco.MjData, leg: LegBinding, local_offset: np.ndarray) -> np.ndarray:
    if len(leg.dof_adrs) == 0:
        return np.zeros(0, dtype=float)

    point_world = foot_point_world(d, leg.calf_body_name, local_offset)
    jacp = np.zeros((3, m.nv), dtype=float)
    jacr = np.zeros((3, m.nv), dtype=float)
...
def store_swing_lift_dirs(m: mujoco.MjModel, d: mujoco.MjData, bindings: ModelBindings, local_offset: np.ndarray) -> None:
    for leg in bindings.leg_bindings:
        leg.swing_lift_dir = estimate_swing_lift_dir(m, d, leg, local_offset)
...
def leg_phase_offset(leg_idx: int) -> float:
    # FL, FR, RL, RR: diagonal pairs in trot.
    offsets = [0.0, 0.5, 0.5, 0.0]
    return offsets[leg_idx]


def swing_phase_alpha(t: float, leg_idx: int, cfg: MPCConfig) -> float:
    """Smooth 0->1->0 swing activation for a leg, aligned with the baseline trot scheduler."""
    cyc = cycle_time(cfg)
...
def build_ctrl_targets(d: mujoco.MjData, bindings: ModelBindings, home_ctrl: np.ndarray, scheduled_contact: np.ndarray, swing_amp: float) -> np.ndarray:
    if home_ctrl.size == 0:
        return home_ctrl
    ...
    for leg_idx, leg in enumerate(bindings.leg_bindings):
        ...
        if bool(scheduled_contact[leg_idx]):
            # For stance legs, track the current joint pose to avoid fighting the torque-level GRF injection.
            target = current_q
        else:
            alpha = swing_phase_alpha(t, leg_idx, MPCConfig())  # placeholder, overwritten below if needed
            raise RuntimeError("build_ctrl_targets requires cfg-aware wrapper; use build_ctrl_targets_with_cfg instead.")
...
def print_binding_summary(bindings: ModelBindings) -> str:
    ...
    for leg in bindings.leg_bindings:
        lines.append(
            f"{leg.leg}: calf={leg.calf_body_name}, joints={leg.joint_ids}, actuators={leg.actuator_ids}, geoms={sorted(leg.geom_ids)}"
        )
    return "\n".join(lines)
