...
```bash
source .venv/bin/activate
python runner_mujoco_smoke.py --model ./mujoco_menagerie/unitree_a1/scene.xml --scenario straight_trot
```
...
```bash
source .venv/bin/activate
python runner_mujoco_smoke.py --model ./mujoco_menagerie/unitree_a1/scene.xml --scenario turn_pi_over_4
```
...
```bash
source .venv/bin/activate
python runner_mujoco_smoke.py \
  --model ./mujoco_menagerie/unitree_a1/scene.xml \
  --scenario straight_trot \
  --headless \
  --output-dir outputs_mujoco/straight_trot
```
...
- At the end, figures are saved under `outputs_mujoco/...`.
