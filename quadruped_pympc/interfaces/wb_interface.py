    ...
    def _process_crawl_recovery(
        ...
        if planted_front_recovery_hold_s > 1e-9:
            ...
            if planted_front_postdrop_recovery_trigger:
                # The best crawl run still collapses almost immediately after
                # late full-contact recovery releases while the same front
                # planned-swing leg remains physically planted. Grant one extra
                # short recovery chunk on that falling edge instead of
                # broadening earlier recovery windows that already proved
                # counterproductive.
                self.full_contact_recovery_remaining_s = max(
                    float(self.full_contact_recovery_remaining_s),
